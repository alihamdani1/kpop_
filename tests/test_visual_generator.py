...
def test_font_size_class_texte_court():
    assert visual_generator._font_size_class("a" * 50) == "text-lg"


def test_font_size_class_texte_moyen():
    assert visual_generator._font_size_class("a" * 150) == "text-md"


def test_font_size_class_texte_long():
    assert visual_generator._font_size_class("a" * 250) == "text-sm"


def _build_html(*, image_bytes: bytes = b"x", tweet_text: str = "Un tweet de test.") -> str:
    return visual_generator.build_html(
        _TEMPLATE_PATH,
        image_bytes=image_bytes,
        tweet_text=tweet_text,
        category_label="RELEASE",
        formatted_date="14 août 2026 · 10h07",
    )


def test_build_html_integre_image_et_texte():
    html = _build_html(image_bytes=b"fake-image-bytes", tweet_text="Un tweet de test.")
    encoded = base64.b64encode(b"fake-image-bytes").decode("ascii")
    assert f"data:image/jpeg;base64,{encoded}" in html
    assert "Un tweet de test." in html
    assert 'class="tweet-text text-lg"' in html


def test_build_html_integre_categorie_et_date():
    html = _build_html()
    assert "RELEASE" in html
    assert "14 août 2026 · 10h07" in html


def test_build_html_echappe_le_html_dans_le_texte():
    """Le texte du tweet n'est jamais du HTML de confiance — un '<' ou '&' littéral ne doit pas
    casser la mise en page ni être interprété comme balise."""
    html = _build_html(tweet_text="Une percée <script> & compagnie")
    assert "<script>" not in html
    assert "&lt;script&gt;" in html


def test_build_html_choisit_la_classe_selon_la_longueur():
    html = _build_html(tweet_text="a" * 250)
    assert 'class="tweet-text text-sm"' in html
